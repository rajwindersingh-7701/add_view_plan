...
if(isset($_POST['submit'])){
	...
	mysqli_query($con,"update withdrwal_cash set r_status='Active',remark='".$_POST['remark']."' where id='".$_POST['id']."'");
	echo '<script>alert("Withdrwal send successfully.");window.location.assign("withdrwal_cash.php");</script>';
}else{
	mysqli_query($con,"update withdrwal_cash set status='failed',r_status='Reject',remark='".$_POST['remark']."' where id='".$_POST['id']."'");
	echo '<script>alert("Withdrwal Reject successfully.");window.location.assign("withdrwal_cash.php");</script>';	
}
}
?>
<!DOCTYPE html>
                	...
                	<div class="col-lg-12">
                        ...
                        <a id="downloadLink" onclick="exportF(this)"><button value="Export to excel" >Export to excel</button></a>
                        	<table class="table table-striped table-bordered"  id="table">
                            	<tr><th colspan="10">Pending Cashback Withdrwal Request</th></tr>
                                <tr>
                                	<th>S.n.</th>
                                ...
                                <?php
									$query = mysqli_query($con,"select * from withdrwal_cash where status='success' and r_status='Pending' order by id");
									if(mysqli_num_rows($query)>0){
										...
										?>
                                        	<tr>
                                                ...
                                                <td><?php if($row['r_status']=='Pending'){ ?>
                            ...
                            <a id="downloadLink" onclick="exportF(this)"><button value="Export to excel" >Export to excel</button></a>
                        	<table class="table table-striped table-bordered"  id="table">
                            	<tr><th colspan="11">Pending Withdrwal Request</th></tr>
                                <tr>
                                	<th>S.n.</th>
                                ...
                                <?php
									$query = mysqli_query($con,"select * from withdrwal_cash where status='Success'  order by id");
									if(mysqli_num_rows($query)>0){
										$i=1;
										while($row=mysqli_fetch_array($query)){
											$us=mysqli_query($con,"select * from user where loginid='".$row['user_id']."'");
