...
if(isset($_POST['submit'])){
	...
	$date=date('Y-m-d');
$wd=mysqli_query($con,"SELECT count(id) FROM `withdrwal` WHERE user_id='$userid' and r_date='$date'");
$wd_r=mysqli_fetch_array($wd);
 $twd=$wd_r['0'];
	if($bank!='' && $ifsc!='' && $mob!='' && $mobile=='10' && $ifcode=='11' && $twd=='0'){
	if($tamnt>=$f_amt && $f_amt>='200'){
...
$result = curl_exec($ch);
	...
	if($status=='success'){
		...
		echo '<script>alert("Successfully Withdrwal");window.location.assign("withdrwal.php");</script>';
		}else{ echo '<script>alert("Withdrwal Pending");window.location.assign("withdrwal.php");</script>';}
	}else{
		echo '<script>alert("Your Balance Is Minimum or Minimum Withdrwal 200/- ");window.location.assign("withdrwal.php");</script>';
	}
	
	}
}
if(isset($_GET['did'])){
	$query = mysqli_query($con,"delete from `withdrwal` where id='".$_GET['did']."'");
		
		
		
		echo '<script>alert("Delete Withdrwal Request.");</script>';}
    ...
    <div class="row mt-3">
      ...
      <div class="col-lg-6">
        <div class="card">
		   ...
		   $ttu=$tu_r['0'];
		   $st=mysqli_query($con,"SELECT * FROM `admin_with` where id='1'");
		   $st_r=mysqli_fetch_array($st);
		   if($st_r['status']=='Open' && $ttu>=2){
		   ?>
          ...
          <div class="alert alert-success">
                            ...
                            <div class="alert alert-info">
                               &nbsp;&nbsp;&nbsp;&nbsp; <a href="#" class="alert-link"><?php echo $st_r['time'];?></a>
                            </div>
          <?php }?>
         </div>
         </div>
