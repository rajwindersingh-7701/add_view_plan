...
if (isset($_GET['select_type'])) {
    if ($_GET['select_type'] != "") {
        ...
        $value = $_GET['v'];
        $q = "SELECT * FROM  `tbl_activation_details` where `user_id`= '$user_id' &&  $column='$value'";
    }
    if (isset($_GET['from'])) {
        $from = date("Y-m-d", strtotime($_GET['from']));
        $q = "SELECT * FROM  `tbl_activation_details` where `user_id`= '$user_id' && datetime >= '$from 00:00:00'";
    }
    if (isset($_GET['to'])) {
    ...
    $totalResult = mysqli_num_rows(mysqli_query($db, "SELECT id FROM  `tbl_activation_details` where `user_id`= '$user_id'"));
}
?>
<style>
...
<!-- Container Panel  -->
<div class="col-lg-9">
    <h2 class="admin-heading bg-offwhite">Activation Detail</h2>
    <!-- Filter -->
    <!-- Filter End -->
    <!-- All Transactions  -->
    <div class="profile-content">
        ...
        <div class="transaction-area">
            ...
            $q = "SELECT * FROM `tbl_activation_details` where `user_id`= '$user_id' ";
            if (isset($_GET['select_type'])) {
                $column = $_GET['select_type'];
                $value = $_GET['v'];
                if ($_GET['select_type'] != "") {
                ...
                <div class="items">
                    <a href="#">
                        <div class="row">
                            ...
                            <div class="col col25p">
                                <span class="name"><?php echo $dataQuery1['package']; ?></span>
                            </div>
                            <div class="col col25p" >
