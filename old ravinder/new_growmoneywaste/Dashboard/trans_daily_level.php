...
if (isset($_GET['select_type'])) {
    if ($_GET['select_type'] != "") {
        ...
        $value = $_GET['v'];
        $q = "SELECT * FROM  `wallet` where `userId`= '$userId' && transaction_type ='royalty' &&  $column='$value'";
    }
    if (isset($_GET['from'])) {
        $from = date("Y-m-d", strtotime($_GET['from']));
        $q = "SELECT * FROM `wallet` where `userId`= '$userId' && transaction_type ='royalty' && datetime >= '$from 00:00:00'";
    }
    if (isset($_GET['to'])) {
    ...
    $totalResult = mysqli_num_rows(mysqli_query($db, "SELECT id FROM  `wallet` where `userId`= '$userId' && transaction_type ='royalty'"));
}
?>

<!-- Container Panel  -->
<div class="col-lg-9">
    <h2 class="admin-heading bg-offwhite">Royalty Bonus</h2>
    <!-- Filter -->
    <!-- Filter End -->
    <!-- All Transactions  -->
    <div class="profile-content">
        ...
        <div class="transaction-area">
                    ...
                    $q = "SELECT * FROM `wallet` where `userId`= '$userId' && transaction_type ='royalty' ";
                    if (isset($_GET['select_type'])) {
                        $column = $_GET['select_type'];
                        $value = $_GET['v'];
                        if ($_GET['select_type'] != "") {
