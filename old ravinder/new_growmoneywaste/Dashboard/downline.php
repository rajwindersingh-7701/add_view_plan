...
if (isset($_GET['select_type'])) {
    if ($_GET['select_type'] != "") {
        ...
        $value = $_GET['v'];
        $str = 'select_type=&';
        $q = "SELECT * FROM  `downline_count` where `tag_sponsor`= '$user_id'  &&  $column='$value'";
    }

    if (isset($_GET['from'])) {
        $str = 'select_type=&from=' . $_GET['from'] . '&';
        $fromDate = date("Y-m-d", strtotime($_GET['from']));
        $q = "SELECT * FROM `downline_count` where `tag_sponsor`= '$user_id'  and date >= '" . $fromDate . " 00:00:00'";
//        echo $q;die;
    }
    ...
    $totalResult = mysqli_num_rows(mysqli_query($db, "SELECT id FROM  `downline_count` where `tag_sponsor`= '$user_id'"));
//    $leftsmw= mysqli_fetch_array(mysqli_query($db,"SELECT * FROM down_count_total WHERE `userId`='".$userData['id']."'"));
//    $totalResult = $leftsmw['left'];
}
?>
        ...
        <div class="row">
            <div class="col-md-12 border rounded">
                <div class="box box-info">
                    <div class="box-header with-border mt-4 mb-4"><h3 class="box-title p-2"><strong>List of Downline Participant (Total:-<?php echo $totalResult; ?>)</strong></h3></div>
                </div>
                <div class="box-body padding" style="background:#FFFFFF; margin-top:-2%">
                    <div>
                        ...
                        <div class='span12'><button class="orangebuttion btn btn-default2 btn-center mb-3" style="" onclick='selectForm()' >Change Filter</button></div>
                            ...
                            <form action="" method ="get" class="form1" <?php if (isset($_GET['from'])) { ?> style="display:none;"<?php } ?>>
                                ...
                                <div class="row mt-3">
                                    ...
                                    <div class="col-md-3 mb-4">
                                        ...
                                        <select name="select_type" class="form-control" id="select_search" onchange="searchSelect()">
                                    ...
                                    <div class="col-md-4 mb-4">
                                        ...
                                        <div class="form-outline">
                                            <input type="text" name="v" class="form-control" value="<?php if (isset($value)) echo $value; ?>" placeholder="Please enter your keyword" required/>
                                        </div>
                                ...
                                <input type="submit"  class="orangebuttion btn btn-warning" name="submit" value="Search" > 
                                <a href="downline.php"><input class="orangebuttion btn btn-warning" type="button" name="reset" value="Reset" > </a>
                            </form>
                        </div>
                        <script>
                            function selectForm() {
                        ...
                        <form action="" method ="get" class="form2" <?php if (empty($_GET['from'])) { ?> style="display:none;"<?php } ?>>
                            ...
                            <a href="Users.php"><input type="button" class="btn btn-warning" name="reset" value="Reset" > </a>
                        </form>
                    </div>

                    <div >
                        <p class="mt-3"><?php echo "Total Result:- " . $totalResult; ?></p>
                        <ul class="pagination">
                            <li style="">Page </li>
                            <?php
...
$q = "SELECT * FROM `downline_count` where `tag_sponsor`= '$user_id'  && `position`='left' ";
