...
if (isset($_GET['select_type'])) {
    if ($_GET['select_type'] != "") {
        ...
        $value = $_GET['v'];
        $q = "SELECT * FROM  `wallet` where `username`= '$user_id' &&  $column='$value'";
    }
    if (isset($_GET['from'])) {
        $from = date("Y-m-d", strtotime($_GET['from']));
        $q = "SELECT * FROM  `wallet` where `username`= '$user_id' && datetime >= '$from 00:00:00'";
    }
    if (isset($_GET['to'])) {
    ...
    $totalResult = mysqli_num_rows(mysqli_query($db, "SELECT id FROM  `wallet` where `username`= '$user_id'"));
}
?>
<style>
...
<!-- Container Panel  -->
<div class="col-lg-9">
    <h2 class="admin-heading bg-offwhite">E-wallet Request Detail</h2>
    <!-- Filter -->
    <!-- Filter End -->
    <!-- All Transactions  -->
    <div class="profile-content">
        ...
        <div class="transaction-title bg-offwhite">
            <div class="items">
                <div class="row">
                    ...
                    <div class="col col25p" ><span class="">User Id</span></div>
                    <div class="col col25p">Amount</div>
                    <div class="col col25p">Type</div>
                    <div class="col col25p">Description</div>
                    <div class="col col25p">Transaction Type</div>
                    <div class="col col25p">Date</div>
                </div>
            </div>
        </div>
        ...
        <div class="transaction-area">
            ...
            $q = "SELECT * FROM `wallet` where `username`= '$user_id' ";
            if (isset($_GET['select_type'])) {
                $column = $_GET['select_type'];
                $value = $_GET['v'];
                if ($_GET['select_type'] != "") {
...
//$userId = $user["id"];
            ...
            // print_r(mysqli_fetch_array($queryUser));
            // die;
            while ($dataQuery1 = mysqli_fetch_array($queryUser)) {
                $userFromID = $dataQuery1['username'];
                ...
                <div class="items">
                    <a href="#">
                        <div class="row">
                            ...
                            <div class="col col25p">
                                <span class="name"><?php echo ucwords($dataQuery1['type']); ?></span>
                            </div>
                            <div class="col col25p">
                                <span class="name"><?php echo $dataQuery1['description']; ?></span>
                            </div>
                            <div class="col col25p">
                                <span class="name"><?php echo ucwords(str_replace('_',' ',$dataQuery1['transaction_type'])); ?></span>
                            </div>
                            <div class="col col25p" >
