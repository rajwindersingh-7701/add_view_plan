...
if (isset($_GET['select_type'])) {
    if ($_GET['select_type'] != "") {
        ...
        $value = $_GET['v'];
        $str='select_type=&';
        $q = "SELECT * FROM  `downline_count` where `tag_sponsor`= '$user_id' && `position`='left' &&  $column='$value'";
    }

    if (isset($_GET['from'])) {
          $str='select_type=&from='.$_GET['from'].'&';
        $fromDate = date("Y-m-d", strtotime($_GET['from']));
        $q = "SELECT * FROM `downline_count` where `tag_sponsor`= '$user_id' && `position`='left' and date >= '" . $fromDate . " 00:00:00'";
//        echo $q;die;
    }
    ...
    $totalResult = mysqli_num_rows(mysqli_query($db, "SELECT id FROM  `downline_count` where `tag_sponsor`= '$user_id' && `position`='left'"));
