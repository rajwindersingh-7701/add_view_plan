...
if (isset($_GET['select_type'])) {
    if ($_GET['select_type'] != "") {
        ...
        $value = $_GET['v'];
        $q = "SELECT * FROM  `level_downline` where `sponser_by`= '$user_id' and levl='".$_SESSION['lvv']."' and package=1000 &&  $column='$value'";
    }
    if (isset($_GET['from'])) {
        $from = date("Y-m-d", strtotime($_GET['from']));
        $q = "SELECT * FROM  `level_downline` where `sponser_by`= '$user_id' and levl='".$_SESSION['lvv']."'  and package=1000 && datetime >= '$from 00:00:00'";
    }
    if (isset($_GET['to'])) {
    ...
    $totalResult = mysqli_num_rows(mysqli_query($db, "SELECT id FROM  `level_downline` where `sponser_by`= '$user_id' and levl='".$_SESSION['lvv']."'  and package=1000"));
}
?>
<style>
...
<!-- Container Panel  -->
<div class="col-lg-9">
    <h2 class="admin-heading bg-offwhite">Active Team level <?php echo $_SESSION['lvv']; ?></h2>
    <!-- Filter -->
    <!-- Filter End -->
    <!-- All Transactions  -->
    <div class="profile-content">
                ...
                <div class="row">
                    ...
                    <div class="col col25p" ><span class="">UserName</span></div>
                     <div class="col col25p" ><span class="">Phone</span></div>
                      <div class="col col25p" ><span class="">Ad View Status</span></div>
        ...
        <div class="transaction-area">
            ...
            $q = "SELECT * FROM `level_downline` where `sponser_by`= '$user_id' and level='".$_SESSION['lvv']."' and package=1000 ";
//           echo $q;
            if (isset($_GET['select_type'])) {
                $column = $_GET['select_type'];
                $value = $_GET['v'];
...
//$userId = $user["id"];
            ...
            while ($dataQuery1 = mysqli_fetch_array($queryUser)) {
                ...
                $smw = mysqli_fetch_array(mysqli_query($db, "SELECT COUNT(*) as smamnt FROM `wallet` WHERE username='$userFromID' and `transaction_type`='ad' and datetime>'$currentdate'"));
    ...
    <!--                                <span class="date">17</span>
                            ...
                            <div class="col col25p">
                                <span class="name">
                                    ...
                                    <?php if($smw['smamnt']==0){
                                        echo "<span style='color:red'>Ad not view</span>";
                                    }else{
                                          echo "<span style='color:green'>Ad viewed</span>";
                                    }
                                    ?>
                                </span>
                            </div>
