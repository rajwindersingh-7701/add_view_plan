...
die;
    $withid = mysqli_real_escape_string($db, $_POST['withid']);
    $withQuery = mysqli_query($db, "SELECT * FROM  `withdraw` where `id`='$withid' ");
    $resultWith = mysqli_fetch_array($withQuery);
    $uQuery = mysqli_query($db, "SELECT `id`,`phone`,`user_id` FROM  `user` where `id`='" . $resultWith['userId'] . "' ");
    $udata = mysqli_fetch_array($uQuery);
    if (COUNT($resultWith) == 0) {
        echo "Something went wrong";
        die;
    }
    $amount = round($resultWith['amount']);
...
//      print_r($res);
//    print_r("h" . $res->status . "h");
    $resjson = json_decode($res, true);
    $res1 = json_encode($res, true);
//  
//    print_r($resjson);
//    print_r('d' . $resjson->status . 'f');
    if ($resjson['status'] == 1) {
        $tran = $resjson['txnid'];
        $msg = $resjson['message'];
//        echo "UPDATE `withdraw` SET `status`='done',`remarks`='$msg',`res`='$resjson' WHERE `id`='" . $resultWith['id'] . "'";
        mysqli_query($db, "UPDATE `withdraw` SET `status`='done',`remarks`='$msg',`res`='$res1' WHERE `id`='" . $resultWith['id'] . "'");
        mysqli_query($db, "INSERT INTO `meta`(`type`,`text`) VALUES ('error','$res')");
        print_r($resjson['message']);
        echo $msg;
        die;
    } else {
        mysqli_query($db, "INSERT INTO `meta`(`type`,`text`) VALUES ('error','$res1')");
    }
...
if (isset($_POST['epin_send'])) {
    ...
    $packQuery = mysqli_query($db, "SELECT `price` FROM  `package` where `id`='$package' ");
...
//        header("location:../add_fund.php");
        echo "<script type='text/javascript'> document.location = '../epin_send.php'; </script>";
        exit;
    }
    $currentdate = date('Y-m-d H:i:s');
    for ($i = 0; $amount > $i; $i++) {
        $epiHash = $fu->getEpin($db) . $packData['price'];
        mysqli_query($db, "INSERT INTO `epin`(`user_id`,`epin`, `package`,`generated_by`,`package_price`) VALUES ('$userIds','$epiHash','$package','admin','" . $packData['price'] . "')");
    }

    $_SESSION["send"] = array("msg" => "Succefully transfer Epin:- $amount in user id:- $userIds", "status" => "true");
//    header("location:../add_fund.php");
    $baseurl = "sms.gniwebsolutions.com/submitsms.jsp?";
    $msg = "You have recived $amount in Tradewidme account.";
    $phone = $userTresult['phone'];
    $msg = urlencode($msg);
    $url = $baseurl . 'user=' . $userkey . '&&key=' . $key . '&&mobile=' . $userPhone . '&&senderid=' . $senderid . '&&message=' . $msg . '&&accusage=1';
...
if (isset($_REQUEST["ido"])) {
    $ido = $_REQUEST["ido"];
    $data = mysqli_query($db, "SELECT * FROM `user` WHERE id = '$ido'");
...
//    $date = date("Y-m-d H:i:s");
//    $data = mysqli_query($db, "update withdraw set `status`='done',confirmDate='$date' where `id`='$with_id'");
////    header("location:../withdraw.php");
//     echo "<script type='text/javascript'> document.location = '../withdraw.php'; </script>";exit;
//}
//if (isset($_REQUEST["with_id"])) {
//    $with_id = $_REQUEST["userId"];
//    $remarks = $_REQUEST["remarks"];
//    	$withQuery = mysqli_query($db, "SELECT userId,amount FROM  `withdraw` where `id`='$with_id' ");    $resultWith = mysqli_fetch_array($withQuery);
//
//    $userQuery = mysqli_query($db, "SELECT phone FROM  `user` where `id`='" . $resultWith['userId'] . "' ");
//    $resultUser = mysqli_fetch_array($userQuery);		
//
//    $date = date('Y-m-d H:i:s');
...
//    $msg = "Dear associate Your Withdraw request confirmed. It will be deposit in your bank account with in 48 hours. Thanking you Tradewidme Team";
//    $msg = urlencode($msg);
...
//    }    
    ...
    $data = mysqli_query($db, "UPDATE  `user` SET  `name` =  '$name',`email`='$email',`phone`='$phone',`father_name` ='$fname',`dob` =  '$dob',`address` =  '$address' ,`password`='$password' WHERE  `user`.`id` ='$userId'");
    //$newData->userSessionUpdate($db, $userId);
...
if (isset($_POST['change_pass'])) {
    ...
    if ($_POST['current_pass'] == $aminData['password']) {
        mysqli_query($db, "UPDATE  `user` SET  `password` ='" . $_POST['new_pass'] . "' WHERE `id` ='$adminid'");
//        $_SESSION['type'] = true;
