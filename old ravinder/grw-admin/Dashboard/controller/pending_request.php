...
//        }
    ...
    if (isset($_GET['to'])) {
     ...
     $totalResult1 = mysqli_fetch_array(mysqli_query($db,"SELECT count(*) as count FROM `withdraw` where status!='done'")); 
    ...
    if (isset($_GET['select_type'])) {
        ...
        ?> 
                    ...
                    <?php
                                        ...
                                        $quwith = "SELECT sum(amount) as sumwith FROM  `withdraw` where `status`='pending'";
                                          if ($_GET['select_type'] != "") {
...
$q .= "order by acc desc limit $pageResult offset $offset";
//                                echo $q;die;
//$userId = $user["id"];
