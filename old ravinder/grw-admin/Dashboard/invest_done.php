...
if (isset($_GET['select_type'])) {
    ...
    if (isset($_GET['from'])) {
        if ($_GET['from'] != "") {
            $q = "SELECT * FROM  `payment_invest` where status='done' and date >= '" . $_GET['from'] . " 00:00:00'";
            $totalResult = mysqli_num_rows(mysqli_query($db, $q));
        }
    }
    if (isset($_GET['to'])) {
    ...
    $totalResult = mysqli_num_rows(mysqli_query($db, "SELECT id FROM `payment_invest` where status='done'"));
    $totalSum = mysqli_fetch_array(mysqli_query($db, "SELECT SUM(amount) as sm FROM `payment_invest` WHERE status = 'done'"));
}
?>
        ...
        <div class="span12">
            ...
            <div class="widget">
                ...
                <div class="widget-header">
                    ...
                    <div class="title">
                        Fund Request Total Amount(Total:-<?php echo $totalSum['sm']; ?>)
                    </div>
                ...
                <div class="widget-body">
                    ...
                    <div>
                        ...
                        <form action="" method="get" class="form1" <?php if (isset($_GET['from'])) { ?> style="display:none;" <?php } ?>>
                            ...
                            <a href="invest_done.php"><input class="orangebuttion" type="button" name="reset" value="Reset"> </a>
                        </form>
                        <script>
                            function selectForm() {
                                jQuery(".form1").toggle();
                    ...
                    <div>
                        ...
                        if ($totalResult > 100) {
                            ...
                            echo "<li><a href='invest_done.php?page=" . ($page - 1) . "' class='button'>Previous</a></li>";

                            for ($i = $page; $i <= ($page + 2); $i++) {
                                echo "<li><a href='invest_done.php?page=" . $i . "'>" . $i . "</a></li>";
                            };
                            echo "<li><a href='invest_done.php?page=" . ($page + 1) . "' class='button'>NEXT</a></li>";
                            echo "</ul>";
                        ?>
                        <?php } ?>
                    </div>
                    <div class="table-responsive">
                        ...
                        <table id="myTable" class="table table-striped">
                            ...
                            <tbody>
                                ...
                                $q = "SELECT * FROM  `payment_invest` where status='done'";
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td> <?php if ($dataQuery['status'] == 'done') { ?> <p style="color: red;"> <?php echo $dataQuery['status']; ?></p> <?php } else { ?> <p style="color: green;"> <?php echo $dataQuery['status']; ?></p> <?php } ?> </td>
                                        <td><a class="btn btn-default btn-sm" onclick="checkout(<?php echo $dataQuery['id']; ?>)">Confirm</a>
                                            <a class="btn btn-default btn-sm" onclick="checkout1(<?php echo $dataQuery['id']; ?>)">Reject</a>
                                        </td>
