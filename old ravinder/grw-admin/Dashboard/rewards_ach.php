...
if(isset($_GET['select_type'])){
    if($_GET['select_type']!=""){
       ...
       $value = $_GET['v'];
    $q = "SELECT * FROM  `rewards_acheiver` where confirm=''";
  
    }
   
    if(isset($_GET['from'])){
         $q = "SELECT * FROM  `rewards_acheiver` where confirm='' and  date >= '".$_GET['from']." 00:00:00'";
    }
    if(isset($_GET['to'])){
 ...
 $totalResult = mysqli_num_rows(mysqli_query($db,"SELECT * FROM  `rewards_acheiver` where confirm=''")); 
}
?>
...
//$userId = $user["id"];
                                ...
                                while ($dataQuery = mysqli_fetch_array($queryUser)) {
                                   ...
                                   $userresult = mysqli_fetch_array($userquery);
                                    $rvquery = mysqli_query($db, "SELECT * FROM `rewards` where `id`= '".$dataQuery['rewards']."'");
                                   $rvresult = mysqli_fetch_array($rvquery);
                                    ?>
                                    <tr>                                   
                                    ...
                                    <td><?php echo $rvresult['reward']; ?>   </td>
                                        <td>
                                           
                                               <a  onclick="return confirm('Are you sure!')" href="controller/function.php?rewards=<?php echo $dataQuery['id']; ?>" class="btn btn-warning " data-original-title="">done</a>  
                                        </td>
