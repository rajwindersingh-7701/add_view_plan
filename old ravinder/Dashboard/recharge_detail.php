...
if (isset($_GET['select_type'])) {
    if ($_GET['select_type'] != "") {
        ...
        $value = $_GET['v'];
        $q = "SELECT * FROM  `recharge` where `userId`= '$user_id' &&  $column='$value'";
    }
    if (isset($_GET['from'])) {
        $from = date("Y-m-d", strtotime($_GET['from']));
        $q = "SELECT * FROM  `recharge` where `userId`= '$user_id' && datetime >= '$from 00:00:00'";
    }
    if (isset($_GET['to'])) {
    ...
    $totalResult = mysqli_num_rows(mysqli_query($db, "SELECT id FROM  `recharge` where `userId`= '$user_id'"));
}
?>
<style>
...
<!-- Container Panel  -->
<div class="col-lg-9">
    <h2 class="admin-heading bg-offwhite">Recharge Detail</h2>
    <!-- Filter -->
    <!-- Filter End -->
    <!-- All Transactions  -->
    <div class="profile-content">
        ...
        <div class="transaction-title bg-offwhite">
            <div class="items">
                <div class="row">
                    <div class="col col20p" ><span class="">Type</span></div>
                    <div class="col col20p" ><span class="">amount</span></div>
                    <div class="col col20p">Number</div>
        ...
        <div class="transaction-area">
            ...
            $q = "SELECT * FROM `recharge` where `userId`= '$user_id' ";
            if (isset($_GET['select_type'])) {
                $column = $_GET['select_type'];
                $value = $_GET['v'];
                if ($_GET['select_type'] != "") {
...
//$user_id = $user["id"];
            ...
            while ($dataQuery1 = mysqli_fetch_array($queryUser)) {
                ...
                <div class="items">
                    <a href="#">
                        <div class="row">
                            <div class="col col20p" >
                                <span class="name"><?php echo $dataQuery1['reachargeType']; ?></span>
                            </div>
                            <div class="col col20p">
                                <span class="name"><?php echo $dataQuery1['amount']; ?></span>
                            </div>
                            <div class="col col20p">
                                <span class="name"><?php echo $dataQuery1['number']; ?></span>
                            </div>
                            <div class="col col20p">
                                <span class="name"><?php echo $dataQuery1['msg']; ?></span>
                            </div>
                        </div>
                    </a>
                </div>
