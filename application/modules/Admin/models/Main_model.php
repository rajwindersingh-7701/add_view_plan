...
//        echo $this->db->last_query();
    ...
    {
        $this->db->select('ifnull(sum(tbl_users.package_amount),0) as business');
        $this->db->from('tbl_users');
...
//        echo $this->db->last_query();
    ...
    public function leveldelete($table, $id) {
        $this->db->where($id);
        return $this->db->delete($table);
    }

    public function calculate_team($user_id,$level){
    ...
    public function calculateTeam($user_id){
        $this->db->select('ifnull(count(tbl_sponser_count.downline_id),0) as team, tbl_users.paid_status');
        $this->db->from('tbl_users');
        $this->db->join('tbl_sponser_count', 'tbl_users.user_id = tbl_sponser_count.downline_id');
        $this->db->where(array('tbl_sponser_count.user_id' => $user_id ,'tbl_users.paid_status' => 1 ));
        $query = $this->db->get();
        $res = $query->row_array();
        return $res;
    }

    public function deleteCheck($table, $where) {
        $this->db->where($where);
        return $this->db->delete($table);
    }
