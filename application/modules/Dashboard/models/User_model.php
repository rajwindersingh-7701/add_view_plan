...
class User_model extends CI_Model {
    ...
    public function __construct() {
        ...
        // $this->load->database2();
        // $otherdb = $this->load->database('gmtcoina_adasd',true);
    }
...
//        echo $this->db->last_query();
    ...
    public function paid_self_team($user_id) {
     ...
     public function levelReport($where) {
    ...
    public function calculateLevelTeam($user_id,$status,$level){
        $this->db->select('ifnull(count(tbl_sponser_count.downline_id),0) as team,ifnull(sum(tbl_users.package_amount),0) as investment');
        $this->db->from('tbl_users');
        $this->db->join('tbl_sponser_count', 'tbl_users.user_id = tbl_sponser_count.downline_id');
    ...
    public function calculate_my_team($user_id,$level){
        $this->db->select('ifnull(count(tbl_sponser_count.downline_id),0) as team, tbl_users.paid_status');
        $this->db->from('tbl_users');
        $this->db->join('tbl_sponser_count', 'tbl_users.user_id = tbl_sponser_count.downline_id');
    ...
    public function add2($table, $data) {
        $this->db2->insert($table, $data);
        return $this->db2->insert_id();
    }

    // public function get_single_record2($table, $where, $select,$queryshow = false) {
    //     $this->db2->select($select);
    //     $query = $this->db2->get_where($table, $where);
    //     $res = $query->row_array();
    //     if($queryshow == true)
    ...
    // public function get_records2($table, $where, $select) {
    //     $otherdb->select($select);
    //     $query = $this->db2->get_where($table, $where);
    //     $res = $query->result_array();
    //     return $res;
    // }
